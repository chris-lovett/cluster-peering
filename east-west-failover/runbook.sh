...
# Usage:
#   chmod +x runbook.sh
#   CONTEXT_C1=<kubectl-context-cluster-01> \
#   CONTEXT_C2=<kubectl-context-cluster-02> \
#   ./runbook.sh
#
...
# ── Config ────────────────────────────────────────────────────────────────────
CONTEXT_C1="${CONTEXT_C1:-cluster-01}"   # kubectl context for the dialer cluster
CONTEXT_C2="${CONTEXT_C2:-cluster-02}"   # kubectl context for the acceptor cluster
CONSUL_NS="consul"
APP_NS="default"
...
phase1_mesh() {
  ...
  echo "▶ Applying mesh config to cluster-01..."
  c1 apply -f "${DIR}/01-mesh-cluster01.yaml"

  echo "▶ Applying mesh config to cluster-02..."
  c2 apply -f "${DIR}/02-mesh-cluster02.yaml"
...
# Step 2a: Apply PeeringAcceptor to cluster-02 → controller generates token Secret
# Step 2b: Copy the token Secret from cluster-02 to cluster-01
# Step 2c: Apply PeeringDialer to cluster-01 → connection is established
# =============================================================================
phase2_peering() {
  banner "PHASE 2 — Establish Cluster Peering"

  # 2a — Acceptor
  echo "▶ [cluster-02] Creating PeeringAcceptor..."
  c2 apply -f "${DIR}/03-acceptor-cluster02.yaml"
  ...
  echo "✔ Token Secret ready on cluster-02"

  # 2b — Token copy
  echo "▶ Copying peering-token Secret from cluster-02 → cluster-01..."
  ...
  echo "✔ peering-token Secret created on cluster-01"

  # 2c — Dialer
  echo "▶ [cluster-01] Creating PeeringDialer..."
  c1 apply -f "${DIR}/04-dialer-cluster01.yaml"
  ...
  echo
  echo "📋 Peering status on cluster-01:"
  c1 get peeringdialers -n "${CONSUL_NS}"
  echo
  echo "📋 Peering status on cluster-02:"
  c2 get peeringacceptors -n "${CONSUL_NS}"
}

# =============================================================================
# PHASE 3 — Deploy Services
# backend on both clusters, frontend on cluster-01.
# =============================================================================
phase3_services() {
  banner "PHASE 3 — Deploy Services"

  echo "▶ [cluster-02] Deploying backend (failover target)..."
  c2 apply -f "${DIR}/05-backend-cluster02.yaml"

  echo "▶ [cluster-01] Deploying backend (primary)..."
  c1 apply -f "${DIR}/06-backend-cluster01.yaml"

  echo "▶ [cluster-01] Deploying frontend..."
  c1 apply -f "${DIR}/07-frontend-cluster01.yaml"
  ...
  echo
  echo "📋 Pods on cluster-01:"
  c1 get pods -n "${APP_NS}" -l 'app in (frontend,backend)'
  echo
  echo "📋 Pods on cluster-02:"
  c2 get pods -n "${APP_NS}" -l app=backend
}

# =============================================================================
# PHASE 4 — Export Services & Configure Intentions
# cluster-02 exports backend to cluster-01, then allows only frontend@cluster-01.
# =============================================================================
phase4_intentions() {
  banner "PHASE 4 — Exported Services & Intentions"

  echo "▶ [cluster-02] Applying ExportedServices..."
  c2 apply -f "${DIR}/08-exported-services-cluster02.yaml"

  echo "▶ [cluster-02] Applying ServiceIntentions..."
  c2 apply -f "${DIR}/09-intentions-cluster02.yaml"
...
# Apply ServiceResolver with peer failover target on cluster-01.
# =============================================================================
phase5_resolver() {
  banner "PHASE 5 — Failover Policy (ServiceResolver)"

  echo "▶ [cluster-01] Applying ServiceResolver..."
  c1 apply -f "${DIR}/10-service-resolver-cluster01.yaml"
...
phase6_verify() {
  ...
  echo "▶ Sending 3 requests from frontend → backend (expect: cluster-01 PRIMARY)..."
  for i in 1 2 3; do
...
" 2>/dev/null || echo "  [${i}] (raw response — check pod logs)"
  ...
  echo
  echo "✔ Baseline verified. All traffic hitting cluster-01 PRIMARY backend."
}

# =============================================================================
# DEMO — Trigger Failover
# Scale primary backend to 0 → watch traffic shift to cluster-02.
# =============================================================================
demo_failover() {
  banner "DEMO — Trigger East-West Failover"

  echo "▶ Scaling backend to 0 replicas on cluster-01 (simulating outage)..."
  ...
  echo
  echo "▶ Sending 5 requests from frontend (expect: cluster-02 FAILOVER)..."
  FRONTEND_POD=$(c1 get pod -n "${APP_NS}" -l app=frontend -o jsonpath='{.items[0].metadata.name}')
...
" 2>/dev/null || echo "  [${i}] (raw response — check pod logs)"
  ...
  echo
  echo "✔ Traffic is now served by cluster-02 FAILOVER backend."
  echo "   Run demo_failback to restore primary."
}

# =============================================================================
# DEMO — Failback
# Restore primary backend on cluster-01 → traffic shifts back automatically.
# =============================================================================
demo_failback() {
  banner "DEMO — Failback to Primary"

  echo "▶ Scaling backend back to 1 replica on cluster-01..."
  ...
  echo
  echo "▶ Sending 3 requests (expect: cluster-01 PRIMARY)..."
  FRONTEND_POD=$(c1 get pod -n "${APP_NS}" -l app=frontend -o jsonpath='{.items[0].metadata.name}')
  for i in 1 2 3; do
    c1 exec -n "${APP_NS}" "${FRONTEND_POD}" -c frontend -- \
      wget -qO- http://backend 2>/dev/null | python3 -c "
...
" 2>/dev/null || echo "  [${i}] (raw response — check pod logs)"
  ...
  echo "✔ Traffic returned to cluster-01 PRIMARY backend."
}
...
teardown() {
  ...
  echo "▶ [cluster-01] Removing app services..."
  c1 delete -f "${DIR}/10-service-resolver-cluster01.yaml" --ignore-not-found
  c1 delete -f "${DIR}/07-frontend-cluster01.yaml" --ignore-not-found
  c1 delete -f "${DIR}/06-backend-cluster01.yaml" --ignore-not-found
  c1 delete -f "${DIR}/04-dialer-cluster01.yaml" --ignore-not-found
  c1 delete secret peering-token -n "${CONSUL_NS}" --ignore-not-found
  c1 delete -f "${DIR}/01-mesh-cluster01.yaml" --ignore-not-found

  echo "▶ [cluster-02] Removing app services..."
  c2 delete -f "${DIR}/09-intentions-cluster02.yaml" --ignore-not-found
  c2 delete -f "${DIR}/08-exported-services-cluster02.yaml" --ignore-not-found
  c2 delete -f "${DIR}/05-backend-cluster02.yaml" --ignore-not-found
  c2 delete -f "${DIR}/03-acceptor-cluster02.yaml" --ignore-not-found
  c2 delete secret peering-token -n "${CONSUL_NS}" --ignore-not-found
  c2 delete -f "${DIR}/02-mesh-cluster02.yaml" --ignore-not-found
